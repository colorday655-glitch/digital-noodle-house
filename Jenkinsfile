    ...
    stages {
            ...
            steps {
                ...
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'vm-ssh', usernameVariable: 'USER', passwordVariable: 'PASS')]) {
                        sh """
                            sshpass -p '${env.VM_PWD}' scp -o StrictHostKeyChecking=no index.html ${env.VM_USER}@${env.VM_HOST}:${TARGET_DIR}/
                        """
                    }
                }
            }
        ...
        stage('飞书通知') {
            steps {
                echo '发送飞书通知...'
                script {
                    def message = '{"msg_type":"text","content":{"text":"✅ 质检通过，招牌菜品已自动上架正式环境，准备营业！"}}'
                    sh """
